...
export const aboutContent = defineType({
  ...
  fields: [
    ...
    defineField({name: "teamIntroduction", title: "Team introduction", type: "text", rows: 8, group: "website", validation: (Rule) => Rule.required().min(80)}),
    defineField({name: "brandTaglines", title: "Editorial interlude lines", type: "array", group: "website", of: [{type: "string"}], validation: (Rule) => Rule.required().min(6).max(6)}),
    defineField({name: "enquiryTaglines", title: "Enquiry reassurance lines", type: "array", group: "website", of: [{type: "string"}], validation: (Rule) => Rule.required().min(3).max(3)}),
  ],
